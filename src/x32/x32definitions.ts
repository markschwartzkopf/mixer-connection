...
const x32Enums = {
	...
	inputSource: (() => {
		const inputSource: string[] = ['OFF'];
		for (let i = 1; i <= 32; i++)
			inputSource.push('In' + i.toString().padStart(2, '0'));
		...
		for (let i = 1; i <= 16; i++)
			inputSource.push('Bus ' + i.toString().padStart(2, '0'));
		return inputSource;
	})(),
	fx14Type: [
		'HALL',
	...
	outputSource: (() => {
		const outputSource: string[] = ['OFF', 'Main L', 'Main R', 'M/C'];
		for (let i = 1; i <= 16; i++)
			outputSource.push('MixBus ' + i.toString().padStart(2, '0'));
		for (let i = 1; i <= 6; i++) outputSource.push('Matrix ' + i.toString());
		for (let i = 1; i <= 32; i++)
			outputSource.push('DirectOut Ch ' + i.toString().padStart(2, '0'));
		for (let i = 1; i <= 8; i++)
			outputSource.push('DirectOut Aux ' + i.toString());
		for (let i = 1; i <= 4; i++) {
...
const x32UserctrlDefinition: x32DefinitionNode = {
	...
	enc: (() => {
		const enc: x32DefinitionNode = {};
		for (let i = 1; i <= 4; i++)
			enc["'" + i.toString()] = { _type: 'encString' };
		return enc;
	})(),
	btn: (() => {
		const btn: x32DefinitionNode = {};
		for (let i = 5; i <= 12; i++)
			btn["'" + i.toString()] = { _type: 'btnString' };
		return btn;
	})(),
};
...
const x32FilterDefinition: x32DefinitionNode = {
	...
	type: {
		_type: 'enum',
		_enum: ['LC6', 'LC12', 'HC6', 'HC12', '1.0', '2.0', '3.0', '5.0', '10.0'],
	},
	f: {
		_type: 'exponential',
		_min: 20,
...
function x32EqDefinition(bands: number, main?: true): x32DefinitionNode {
	const eq: x32DefinitionNode = { on: { _type: 'enum', _enum: ['OFF', 'ON'] } };
	let type: x32DefinitionLeaf = {
		_type: 'enum',
		_enum: ['LCut', 'LShv', 'PEQ', 'VEQ', 'HShv', 'HCut'],
	};
	if (main)
		type = {
			_type: 'enum',
...
function x32MixSendNode(oddNumber: number, main?: boolean) {
	const typeEnum = ['IN/LC', '<-EQ', 'EQ->', 'PRE', 'POST'];
	if (!main) typeEnum.push('GRP');
	const commonLeaves: x32DefinitionNode = {
		on: { _type: 'enum', _enum: ['OFF', 'ON'] },
		level: { _type: 'level161' },
	};
	const mixSendOdd: x32DefinitionNode = {
		pan: {
			_type: 'linear',
			_min: -100.0,
			_max: 100.0,
...
const x32FxConditional: x32Conditional = {
	...
	FILT: [
		...
		{ _name: 'Mix', _type: 'linear', _min: 0, _max: 100, _step: 2 },
		{
			_name: 'Wave',
			_type: 'enum',
			_enum: ['TRI', 'SIN', 'SAW', 'SAW-', 'RMP', 'SQU', 'RND'],
		},
		{ _name: 'Phase', _type: 'linear', _min: 0, _max: 180, _step: 5 },
		{
			_name: 'Env. Modulation',
	...
	P1A: [
		...
		},
		{
			_name: 'Hi Freq',
			_type: 'enum',
			_enum: ['3k', '4k', '5k', '8k', '10k', '12k', '16k'],
		},
		{
			_name: 'Hi Attenuation',
			_type: 'linear',
			_min: 0,
			_max: 10,
			_step: 0.1,
...
const x32NodeStructure: x32DefinitionNode = {
	config: {
		chlink: (() => {
			const chlink: x32DefinitionNode = {};
			for (let i = 1; i <= 31; i += 2)
				chlink[i + '-' + (i + 1)] = { _type: 'enum', _enum: ['OFF', 'ON'] };
			return chlink;
		})(),
		auxlink: (() => {
			const auxlink: x32DefinitionNode = {};
			for (let i = 1; i <= 7; i += 2)
				auxlink[i + '-' + (i + 1)] = { _type: 'enum', _enum: ['OFF', 'ON'] };
			return auxlink;
		})(),
		fxlink: (() => {
			const fxlink: x32DefinitionNode = {};
			for (let i = 1; i <= 7; i += 2)
				fxlink[i + '-' + (i + 1)] = { _type: 'enum', _enum: ['OFF', 'ON'] };
			return fxlink;
		})(),
		buslink: (() => {
			const buslink: x32DefinitionNode = {};
			for (let i = 1; i <= 15; i += 2)
				buslink[i + '-' + (i + 1)] = { _type: 'enum', _enum: ['OFF', 'ON'] };
			return buslink;
		})(),
		mtxlink: (() => {
			const mtxlink: x32DefinitionNode = {};
			for (let i = 1; i <= 5; i += 2)
				mtxlink[i + '-' + (i + 1)] = { _type: 'enum', _enum: ['OFF', 'ON'] };
			return mtxlink;
		})(),
		mute: (() => {
			const mute: x32DefinitionNode = {};
			for (let i = 1; i <= 6; i++)
				mute["'" + i.toString()] = { _type: 'enum', _enum: ['OFF', 'ON'] };
			return mute;
		})(),
		linkcfg: {
		...
		solo: {
			...
			source: {
				_type: 'enum',
				_enum: ['OFF', 'LR', 'LR+C', 'LRPFL', 'LRAFL', 'AUX56', 'AUX78'],
			},
			sourcetrim: {
				_type: 'linear',
				_min: -18,
		...
		dp48: {
			...
			assign: (() => {
				const assign: x32DefinitionNode = {};
				for (let i = 1; i <= 48; i++)
			...
			link: (() => {
				const link: x32DefinitionNode = {};
				for (let i = 1; i <= 24; i++)
					link["'" + i.toString().padStart(2, '0')] = { _type: 'int', _max: 1 };
				return link;
			})(),
			grpname: (() => {
				const assign: x32DefinitionNode = {};
	...
	ch: (() => {
		const ch: x32DefinitionNode = {};
		for (let i = 1; i <= 32; i++)
			ch["'" + i.toString().padStart(2, '0')] = x32ChannelDefinition;
		return ch;
	})(),
	auxin: (() => {
		const aux: x32DefinitionNode = {};
		for (let i = 1; i <= 8; i++)
			aux["'" + i.toString().padStart(2, '0')] = x32AuxDefinition;
		return aux;
	})(),
	fxrtn: (() => {
		const fxrtn: x32DefinitionNode = {};
		for (let i = 1; i <= 8; i++)
			fxrtn["'" + i.toString().padStart(2, '0')] = x32FxrtnDefinition;
		return fxrtn;
	})(),
	bus: (() => {
		const bus: x32DefinitionNode = {};
		for (let i = 1; i <= 16; i++)
			bus["'" + i.toString().padStart(2, '0')] = x32BusDefinition;
		return bus;
	})(),
	mtx: (() => {
		const mtx: x32DefinitionNode = {};
		for (let i = 1; i <= 6; i++)
			mtx["'" + i.toString().padStart(2, '0')] = x32MtxDefinition;
		return mtx;
	})(),
	main: {
		st: {
	...
	outputs: {
		...
		aux: (() => {
			const aux: x32DefinitionNode = {};
			for (let i = 1; i <= 6; i++)
				aux["'" + i.toString().padStart(2, '0')] = x32OutputAuxDefinition;
			return aux;
		})(),
		p16: (() => {
			const p16: x32DefinitionNode = {};
			for (let i = 1; i <= 16; i++)
				p16["'" + i.toString().padStart(2, '0')] = {
					...
					iQ: {
						...
						speaker: {
							_type: 'enum',
							_enum: ['none', 'iQ8', 'iQ10', 'iQ12', 'iQ15', 'iQ15B', 'iQ18B'],
						},
						eq: {
							_type: 'enum',
							_enum: ['Linear', 'Live', 'Speech', 'Playback', 'User'],
						},
		...
		aes: (() => {
			const aes: x32DefinitionNode = {};
			for (let i = 1; i <= 2; i++)
				aes["'" + i.toString().padStart(2, '0')] = x32OutputAuxDefinition;
			return aes;
		})(),
		rec: (() => {
			const rec: x32DefinitionNode = {};
			for (let i = 1; i <= 2; i++)
	...
	headamp: (() => {
		const headamp: x32DefinitionNode = {};
		for (let i = 0; i <= 127; i++)
